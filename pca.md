...
```
    projection(M): Get the projection matrix (of size (d, p)). Each column of the projection matrix corresponds to a principal component.
...
Here we see the density of the projections from the chain to illustrate the uncertainty in the projections. We can see quite clearly that it is possible to separate Setosa from Versicolor and Virginica species, but that the latter two cannot be clearly separated based on the pPCA projection. This can be shown even more clearly by using a kernel density estimate and plotting the contours of that estimate.

```julia
# kernel density estimate
using KernelDensity
dens = kde((vec(scores[1, :, :]), vec(scores[2, :, :])))
